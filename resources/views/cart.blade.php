    ...
    @else
        <div class="container w-100">
            @foreach ($produits as $produit)
                <div class="col-9 py-3 mb-5 d-flex mx-auto">
                    <div
                        style="max-height:100px;max-width:100px;background-repeat:no-repeat;background-size:cover;background-image: url({{ asset('storage/' . $produit['Image']) }})">
                    </div>
                    ...
                    <form action="{{ route('produits.addtocart', ['produit' => $produit->id]) }}" method="POST">
                        @csrf
                        <button type="submit" class=" m nav-link btn btn-info text-white me-2">Delete</button>
                    </form>
                </div>
            @endforeach
        </div>
